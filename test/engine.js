...
asyncTest("rules and plugins exposed", function(){
	...
	setTimeout(function(){
		...
		ok(g.window.Hitch.getPlugins(), "Plugins are exposed");
		equals(g.window.Hitch.getPlugins().length, 1, "Plugin count is 1 on initialization");
		start();
	},200);
});
...
test("plugins contain default set", function(){
	...
	var g = helper();
	setTimeout(function(){
		var pluginsAny = g.window.Hitch.getPlugins()[0];
		equals(g.window.Hitch.getPluginNames()[0], '-hitch-any', "-hitch-any shows up in names");
...
asyncTest("false-return registered with link", function(){
	var g = helper(
		'<link type="text/css" href="support/libs/fake.css" rel="stylesheet" -hitch-interpret="true"></link>'
	);
	setTimeout(function(){
		setTimeout(function(){
...
asyncTest("false-return unregistered no breaking", function(){
	var g = helper('<style -hitch-interpret="true"> div:-false-return() { color: red; } </style>');
	setTimeout(function(){		
