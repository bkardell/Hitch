var Hitch = (function(){
    ...
    var	vendor,
		...
		searchUpward = function(el,q,last){
			...
			if(el.tagName !== 'BODY' && q.scan !== ''){
				for(i in q.index){ // walk through each one in the index
					if(q.index.hasOwnProperty(i)){
						try{
							if(matchFn(el,i)){ // Do we match this one?
								for(f in q.index[i].filters){	// walk through each filter
									...
									for(a=0;a<tmp.length;a++){
										...
										d = tmp[a].cid; 
										if(x){ 
											addMangledClass(el,d);
										}else{ 
											removeMangledClass(el,d); 
										}
									}
								}
							}
					...
					for(var c=0;c<potential.length;c++){
						...
						for(var filterName in fs){
							...
							for(var i=0;i<tests.length;i++){
								...
								if(x){
									addMangledClass(t,tests[i].cid);
								}else{
									removeMangledClass(t,tests[i].cid);
								}
							}
						}
					}
		...
		filters = {
			...
			init: function(){
				...
				if(document.body.addEventListener){ // IE8 grafts attribute modification... what about tree mod?
	...
	return { 
		...
		getPluginNames: function(){
			var ret=[], i=0;
			for(;i<cssplugin_selectors.length;i++){
