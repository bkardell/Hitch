var Hitch = (function(){
	var 
		...
		// performance object used to measure DOM actions
		...
		plugins = [],
		// querySelectorAll from provided element
			...
			if(el.tagName !== 'BODY' && q.scan !== ''){
				for(var i in q.index){ // walk through each one in the index
					try{
						...
						// TODO: Need to find a nice way to message these DOM related probs.
		...
		testSubtree = function(t,isInit){
			...
			if(!subtreesemaphor){
				...
				try{
					...
					// TODO: Need a nice way to message DOM issues
		...
		hitches = {
			...
			parse:   function(){
				var m = {rules: [], segIndex: {}, plugins: []}, i, n;
				for(i=0;i<rules.length;i++){
					m.rules = m.rules.concat(rules[i].rules);
					for(n in rules[i].segIndex){
	...
	// TODO: These are never used when hitch-adapter is in play?
	try{
	...
	return { 
		...
		add: function(h){
			...
			for(i=0;i<descs.length;i++){
				if(descs[i].init && descs[i].type && descs[i].type === 'html'){
					Hitch.dom.addSheet(":" + descs[i].name + "(){}",true);
				}
				if(descs[i]['const']){
					consts.push(descs[i]['const']);
				}
			}
		},
