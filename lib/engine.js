var Hitch = (function(){
    ...
    var	
		...
		apply = function(filterName,potential,t,test,n){
			try{
				...
				x = filters.map[filterName].filter(n,test.args,{siblings: n.parentNode.children, location: normalizedUrl() });
				if(x){ 
		...
		filters = {
			...
			registerFilter: function(alias, base, filter, init){
				if(filter){
					filters.map[":" + alias] = {  filter: filter, base: base, init: init };
				}
			}
		};
	...
	return { 
		...
		getPlugins: function(){ return plugins; }
	};
}());
