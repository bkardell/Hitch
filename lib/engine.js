var Hitch = (function(){
	...
	var 
		...
		hitches = {
			...
			wire: function(){
				var applyStateRules, delegateStateRules;
				applyStateRules = function(hitchName,segName,seg,origElement,toggleType,type){
					setTimeout(function(){
						var x, potentialMatches, matchesToggle = (toggleType==='out') ? seg.args.replace(":" + type, "") : seg.args;

						// it should match 'in' and the selector or 'out' and the selector sans ':hover'
						if(matchesSelector(origElement,matchesToggle.replace(/,$/,""))){
						...
						try{
							if(seg.hitchState[type].length>0){
								for(x=0;x<seg.hitchState[type].length;x++){
									...
									// everything from here up in fnb really should be precomputed and updated when segIndex is....
									for(i=0;i<potentialSegs.length;i++){
										fna(seg.hitchState[type][x].hitch, segName, potentialSegs[i],e.target,toggleType,type);
									}	
								}
							}
						}catch(ex){
				...
				document[Hitch.events.getAttacherFnName()]('mouseover',function(e){fnb(e,"in","hover");});
				document[Hitch.events.getAttacherFnName()]('mouseout',function(e){fnb(e,"out","hover");});
				document[Hitch.events.getAttacherFnName()]('focus',function(e){fnb(e,"in","focus");},true);
				document[Hitch.events.getAttacherFnName()]('blur',function(e){fnb(e,"out","focus");},true);
