var Hitch = (function(){
	...
	var 
		...
		testSubtree = function(t){
			var el = t.target || t;
			scanSubtree(el,function(){
				ready(function(){
					realTest(el || document.getElementsByTagName('body')[0]);
				});
			});
		},
	    ...
	    scanForWidgets = function(root,complete){
			var resources = (root || document).querySelectorAll('[x-hitch-widget],[data-hitch-widget]'),
				i,
				requires = [],
				url, 
				r;
		...
		scanCSS = function(root, complete){
			...
			resources = (root||document).querySelectorAll('[x-hitch-interpret]:not([data-hitch-parsed]),[data-hitch-interpret]:not([data-hitch-parsed])');
			for(i=0;i<resources.length;i++){
		...
		scanSubtree = function(root,cb){
			scanForWidgets(root,function(){
				scanCSS(root, function(){ cb(); });
			});
		},
	...
	return { 
		...
		init: function(){ ready(hitches.init); },
		...
		add: function(h){
			...
			for(i=0;i<descs.length;i++){
				...
				if(hitchDesc.init && hitchDesc.type && hitchDesc.type === 'html'){
					loads.push({"inline": ":" + hitchDesc.name + "(){}"});
