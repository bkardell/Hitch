var HitchCompiler = function(src, finished){
	...
	var getSegIndex = function(rulez){
		...
		for(i = 0; i < rulez.length; i++){
			...
			for(x = 0; x < rule.segments.length; x++){
				...
				if(segment.filter){
					...
					joined = (segments.length > 0 && segments[0] !== '') ? segments.join(' ') : base;
					
					// assure there is an index for the joined segments
					if(!segIndex[joined]){
						segIndex[joined] = { hitches: {}, orig: rule.rule.split("{")[0] };
					}
					// assure there are filters for the joined segments
					if(!segIndex[joined].hitches[segment.filter]){
						segIndex[joined].hitches[segment.filter] = [];
					}
					
					segIndex[joined].hitches[segment.filter].push({
						sid: x, 
						rid: i, 
						cid: segment.cid, 
	...
	var transformToNative = function(selector, compiledRule){
		var
			...
			pastFilterSelector, 
			temp = trimmedString(selector).replace(hasFilterRegEx(), function(m,i,s){
			var 
			...
			if(hasFilterRegEx().test(m.substring(m.indexOf("(")+1))){
	...
	src = src.replace(/\@-hitch-requires[^\;]*\;/g, function(m,i,s){
		...
		// TODO: This is in 2 places (here and adapter)
	...
	var compile = function(){
		...
		// initialize with existing constants from previously defined plugins
		constants = Hitch.getConsts();
		
		// Process source rules for constants
		src = src.replace(/\@-hitch-const[^\;]*\;/g, function(m,i,s){
			var parts = m.split(/\s|\;/g), name, replacement;
			parts.shift();
			name = parts.shift();
			replacement = parts.join(' ');
			constants.push({name: name, replaceWith: replacement});
			// TODO: Is base really the full replacement array or is it just the selector?
			Hitch.add({name: name, base: replacement});
			return '';
		});
		...
		for(var i = 0; i < input.length; i++){
			...
			if(ruleSelector !== '*' && hasFilterRegEx().test(ruleSelector)){
	...
	// TODO: This is a bit confusing as 'requires' potentially is empty
	// and only by #load falling-through to #compile does any of this work.
	Hitch.resources.load(requires, null, 'script', null, compile);
