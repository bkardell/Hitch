var cssPluginsCompiler = function(src, finished){
	...
	var unGroupSelector = function(match, index, selector, native){
		var unGrouped = selector.split(',');
		// if there is no grouping - return normally
		if(unGrouped.length === 1) { 
			return selector.substring(0, index) + native; 
		} else {
			// counter of characters looped through groups
			var scanned = 0;
			for(var i = 0; i < unGrouped.length; i++){
				// position to cut based on real index and scanned
				// minus the index because of the dropped commas
				...
				if(index < scanned){
					return unGrouped[i].substring(0, cut) + native;
				}
			}
		}
	};
	...
	var transformToNative = function(selector, compiledRule){
		return trimmedString(selector).replace(reHasFn, function(m,i,s){
			...
			compiledRule.segments.push({
				"selector": unGroupSelector(m, i, s, nativeSelector),
	...
	$.when.apply($,pluginSources).then(function(pluginSource){
		...
		for(var i = 0; i < input.length; i++){
			...
			if(ruleSelector !== '*' && reHasFilter.test(ruleSelector)){
				...
				compiledRule.rule = comment + ruleSelector + ruleBody;
			}else{
