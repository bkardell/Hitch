(function(hitch){
		...
		{ 
			...
			filter: function(match, argsString){
				...
				for(i=0;i<pts.length;i++){
				   if(hitch.matchesSelector(match, argsString)){
				        ct++;
		...
		{
			/* emulates :contains() as previously specified in CSS3 */
			/* Ominous .... http://www.w3.org/TR/selectors/#content-selectors */
			name: "-hitch-contains",
			base: "",
			filter: function(match, argsString){
				if (argsString.charAt(0) === '/' && argsString.charAt(argsString.length-1) === '/') {
					argsString = new RegExp(argsString.slice(1,-1));
					return match.innerText && argsString.test(match.innerText);
				} 
				return match.innerText && match.innerText === argsString;
			}
		},
		{
			/* 
