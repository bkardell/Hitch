...
Sizzle.matchesSelector = function( elem, expr ) {
	...
	if (!Sizzle.needsSizzle[expr] && support.matchesSelector && documentIsHTML &&
...
;(function (Hitch){
	...
	*/
	...
	HTMLElement.prototype.Hitch = function () {
		return {
			...
			matchesSelector: function (selector, mark) {
				var result;
				Sizzle.__hitchMarkMatch = mark;
				result = Sizzle.matchesSelector(this, selector);
				Sizzle.__hitchMarkMatch = false;
				return result;
			}.bind(this)
		};
	};
}(Hitch || {}));;(function (window, Hitch) {
	...
	var getPotentiallyMatchingRules = function (subj) {
		if (typeof quickSubjectMap[subj] === "undefined") {
			quickSubjectMap[subj] = [];
			Hitch.selectors.unique.forEach(function (selector) {
				if (selector.indexOf(subj) !== -1) {
					quickSubjectMap[subj].push(selector);
				}
			});
		}
		return quickSubjectMap[subj];
	}

	var getMatchingSubjects = function (el) {
	   var rules = [];
	   Hitch.selectors.subjects.forEach(function (subj) {
	       var firstChar, okrules = [];
	        if (Sizzle.matchesSelector(el, subj)) {
	        	rules = getPotentiallyMatchingRules(subj);
	        	/*
	           firstChar = subj.charAt(0);
	           if (firstChar === '[') {
	               okrules = Hitch.selectors.map.ATTR[subj];
	           } else if (firstChar === ".") {
	               okrules = Hitch.selectors.map.CLASS[subj];
	           } else {
	               okrules = Hitch.selectors.map.TAG[subj];
	           }
	           if (okrules) {
	               rules = rules.concat(okrules);
	           }*/
	       }
	   });
	   return rules;
    ...
    var processAddedElements = function (added) {
    	var elsWeCareAbout = Hitch.selectors.subjects.toString();
    	//setTimeout(function (){
			added.forEach(function (el) {
			            var i = 0, hitchedEl, selectors = Hitch.selectors.unique, len = selectors.length;
			            if (el.nodeType === 1) {
			                hitchedEl = el.Hitch();

			                //Roughly 18% of time is spent here on html5 doc... can we do better?
			                if (Sizzle.matchesSelector(el, elsWeCareAbout)) {
			                   var rulz = getMatchingSubjects(el);
			                   for (i=0;i<rulz.length;i++) {

			                    //Hitch.selectors.unique.forEach(function (selector) {
			                    	// The problem here is descendant selectors, really we only want to test elements
			                        hitchedEl.matchesSelector(rulz[i], true); // || el.parentElement.Hitch().matchesSelector(selectors[i], true);
			                    //});
			                    }
			                }
			            }
    ...
    var processMutations = function (mutations) {
    	var i = 0, len = mutations.length, targetedRules, elementsToTest, now = Date.now(), added;

 		for ( ; i<len; i++ ) {
 			mutation = mutations[i];
            // This seems like always the right thing to do for these...
            added = Array.prototype.slice.call(mutation.addedNodes);

            added.forEach(function (item) { addedQueue.push(item); })
            if (addedQueue.length === bucketSize || ((now - lastCheck) > 100 * open)) {
            	processAddedElements(addedQueue);
            	addedQueue = [];
            } else {
            	if (flushTimeout) {
    ...
    var open = 0;
	var balanceNodeList = function (nodeList, bucketMax) {
	    var tmp, rst = [], arr = Array.prototype.slice.call(nodeList), remaining = arr.length, start = 0;
	    while ( remaining > 0 ) {
	        tmp = arr.slice(start, (bucketMax > remaining) ? bucketMax : remaining);
	        start += tmp.length;
	        remaining -= bucketMax;
	        rst.push(tmp);
	    }
    ...
    Hitch.observe = function () {
        ...
        if (supportParseMutations && !mutationObserver) {
            ...
            observer = new MutationObserver(function (mutations) {
                ...
                var balancedList = balanceNodeList(mutations, bucketSize);

                balancedList.forEach(function (xx, i) {
                	open++;
                	setTimeout(function () { processMutations(xx); open--; }, delay);
                });

                /*
               	var queue = [], lastIndex = mutations.length -1;
               	mutations.forEach(function (mutation, i) {
               		queue.push(mutation);
               		if (i === 150 || i === lastIndex) {
               			delay = ((open + 1) > 50) ? 200 + open: open * 70;
               			setTimeout(getProcessor(queue), (delay));
               			i = 0;
               			queue = [];
               		}
               	});*/
            ...
            document.addEventListener("DOMContentLoaded", function () {
                ...
                // TODO: Add queue/async as above
                processAddedElements(elementsToTest);
                /*
                elementsToTest.forEach(function (el) {
