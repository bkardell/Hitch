...
// EXPOSE
  ...
  var plugins = [], pluginsCounter = 0, constants = [], compiled, rules = [];
      ...
      */
      ...
      var registerSelector = function (selector) {
            var tokens = Sizzle.tokenize(selector), next, j = 0, compiledMatchers;
...
;(function (Hitch){
  ...
  var head = document.getElementsByTagName('head')[0], 
      body, /* we'll initialize this as it comes available */
      addClass, 
      removeClass, 
      parseClasses;

  parseClasses = function(e){ 
  ...
  Hitch.dom = {
    addMangledClass: function(t,i){
      ...
      for(var x=0;x<e.length;x++){
        addClass((e[x].target || e[x]), Hitch.prefix + i);
      }
    }, 
    removeMangledClass: function(t,i){
      var e = (t instanceof NodeList) ? t : [t];
      for(var x=0;x<e.length;x++){
        removeClass((e[x].target || e[x]), Hitch.prefix + i);
      }
    }
  };
              ...
              if (mutation.type === "attributes") {
                if (mutation.attributeName === "class") {
                 ...
                 targetedRules.forEach(function (selector) { 
                  mutation.target.Hitch().matchesSelector(selector, true);
                });
              }
